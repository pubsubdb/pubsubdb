...
PubSubDB delivers durable microservice orchestration like [Temporal](https://temporal.io). But PubSubDB is fundamentally different from Temporal in that the engine is an emergent property of the system design. *It is an operational outcome of the data journaling process.* PubSubDB can run millions of simultaneous workflows using a single Redis instance at Redis's unmatched speed and scale.
