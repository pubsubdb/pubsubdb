...
class SerializerService {
    static flattenHierarchy(obj, prefix = '') {
        const result = {};
    ...
    static objectToArray(obj) {
        ...
        function isSequentialNumericKeys(o) {
            let index = 0;
            let sequential = true;
            for (const key in o) {
    ...
    static restoreHierarchy(obj) {
        ...
        for (const key in obj) {
            ...
            for (let i = 0; i < keys.length; i++) {
                if (i === keys.length - 1) {
                    current[keys[i]] = SerializerService.convertTypes(obj[key]);
                }
                else {
