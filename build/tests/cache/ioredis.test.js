"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const ioredis_1 = require("../../cache/ioredis");
describe('RedisConnection', () => {
    afterEach(async () => {
        await ioredis_1.RedisConnection.disconnectAll();
    });
    it('should create a connection to Redis', async () => {
        const redisConnection = await ioredis_1.RedisConnection.getConnection('test-connection');
        expect(redisConnection).toBeInstanceOf(ioredis_1.RedisConnection);
        expect(await redisConnection.getClient()).not.toBeNull();
    });
    it('should disconnect from Redis', async () => {
        const redisConnection = await ioredis_1.RedisConnection.getConnection('test-connection');
        await redisConnection.disconnect();
        expect(redisConnection.getClient()).rejects.toThrow('Redis client is not connected');
    });
    it('should disconnect all instances', async () => {
        await ioredis_1.RedisConnection.getConnection('test-connection-1');
        await ioredis_1.RedisConnection.getConnection('test-connection-2');
        await ioredis_1.RedisConnection.disconnectAll();
        expect(ioredis_1.RedisConnection['instances'].size).toBe(0);
    });
    it('should set and get a value from Redis', async () => {
        const redisConnection = await ioredis_1.RedisConnection.getConnection('test-connection');
        const redisClient = await redisConnection.getClient();
        await redisClient.set('test-key', 'test-value');
        const val = await redisClient.get('test-key');
        expect(val).toBe('test-value');
    });
    it('publishes and subscribes', async () => {
        const publisher = await ioredis_1.RedisConnection.getConnection('publisher');
        const subscriber = await ioredis_1.RedisConnection.getConnection('subscriber');
        const publisherClient = await publisher.getClient();
        const subscriberClient = await subscriber.getClient();
        await subscriberClient.subscribe('article', (err, count) => {
            //expect(message).toBe('message');
        });
        subscriberClient.on('message', (channel, message) => {
            expect(message).toBe('message');
        });
        await publisherClient.publish('article', 'message');
    });
});
